...
| `INV_SITE_WP_CATEGORY_CONTRADICTION` | warn | at `WP_DECIDED`+, the favoured side's *displayed* category results are not a minority — i.e. the scoreboard does not contradict its own WP | **the site shows a near-certain winner while its own cells hand the majority to the other side.** Deliberately a warn (a decisive WP can legitimately lead a current deficit projected to flip), but at ≥`WP_DECIDED` it is almost always a stale rate/QS cell worth eyeballing. Complements `INV_SITE_DB_MISMATCH`, which **cannot** see this: that check skips the derived rate cats (`_LIVE_RECON_CATS`), which is exactly where the divergence hides. Added after 2026-07-27 m96 (WP 100% Norsemen, cells showed Bear 5-4 via a settle-stale OPS + an un-credited QS). Its second real fire was **2026-08-24 m120, ×57 occurrences over one 4h40m episode** — the `_norm_name` collision that flipped OPS (see `INV_SETTLED_RATE_DRIFT`, which was added as the error-grade backstop for that class). Read `occurrences` as *duration*, not spread: 57 ticks was one matchup, one category, one episode — the counting lesson from the `INV_SITE_QS_OVERCREDIT` post-mortem. |
